...
import com.zrhx.base.network.exception.ExceptionEngine;
...
public class ContactFragment extends AbstractAutoListFragment<Contact> {
    ...
    @Override
    protected void getData(int i, @NonNull RequestAction<Contact> requestAction) {
        IMUserConversation.loadContact()
                          .subscribe(requestAction::onRequestSuccess,
                                  throwable -> requestAction.onRequestFailed(ExceptionEngine.handleMessage(throwable)));
    }
}
